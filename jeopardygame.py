import flet as ft
import flet_audio as fta

questions = {
   "General Knowledge": {
       100: (
           "What is the capital of France?",
           "paris",
           "capitaloffrance.jpg"),
       200: (
           "What is the largest ocean in the world?",
           "pacific",
           "largestocean.jpg"),
       300: (
           "How many continents are there?",
           "7",
           "continents7.jpg"),
       400: (
           "Which planet is known as the Red Planet?",
           "mars",
           "redplanet.jpg"),
       500: (
           "What gas do plants absorb from the atmosphere?",
           "carbon dioxide",
           "atmosphereplants.jpg")
   },
   "Literature": {
       100: (
           "Who wrote Romeo and Juliet?",
           "william shakespeare",
           "romeoandjuliet.jpg"),
       200: (
           "Who wrote Don Quixote?",
           "miguel de cervantes",
           "donquixote.jpg"),
       300: (
           "Who is the author of Harry Potter?",
           "j.k. rowling",
           "harrypotter.jpg"),
       400: (
           "What is the name of the wizard in The Lord of the Rings?",
           "gandalf",
           "lordoftherings.jpg"),
       500: (
           "Who wrote One Hundred Years of Solitude?",
           "gabriel garcia marquez",
           "GabrielGarcia.jpg")
   },
   "Sports": {
       100: (
           "How many players are on a soccer team?",
           "11",
           "soccer.jpg"),
       200: (
           "Which country won the 2022 FIFA World Cup?",
           "argentina",
           "fifaworldcup.jpg"),
       300: (
           "How many points is a free throw worth in basketball?",
           "1",
           "basketball.jpg"),
       400: (
           "Which sport uses a racket and a shuttlecock?",
           "badminton",
           "batmitton.jpg"),
       500: (
           "In which sport is the Super Bowl played?",
           "football",
           "superbowl.jpg")
   },
   "Colors": {
       100: (
           "What color is the sky on a clear day?",
           "blue",
           "blue.jpg"),
       200: (
           "What color do you get when you mix blue and yellow?",
           "green",
           "green.jpg"),
       300: (
           "What color has the shortest wavelength in visible light?",
           "violet",
           "violet.jpg"),
       400: (
           "What color is complementary to blue on the color wheel?",
           "orange",
           "orange.jpg"),
       500: (
           "Which color is not part of the rainbow: pink, indigo, or violet?",
           "pink",
           "rainbowpink.jpg")
   }
}

puntos = 0
usadas = []
juego = True

def main(page: ft.Page):
   audio = fta.Audio(
       src="audio/bgmusic.mp4",
       autoplay=True,
       volume=1
   )
   right_sound = fta.Audio(
       src="audio/right.mp4",
       volume=1
   )
   wrong_sound = fta.Audio(
       src="audio/wrong.mp4",
       volume=1
   )
   global puntos
   global juego
   page.services.append(audio)
   page.services.append(right_sound)
   page.services.append(wrong_sound)
   page.title = "Jeopardy"
   page.theme_mode = ft.ThemeMode.DARK
   page.window_width = 1000
   page.window_height = 750
   page.bgcolor = "#001f3f"
   page.padding = 20
   page.horizontal_alignment = ft.CrossAxisAlignment.CENTER
   page.scroll = "auto"
   titulo = ft.Text(
       "JEOPARDY",
       size=42,
       weight="bold",
       color="yellow"
   )
   score = ft.Text(
       f"Score: {puntos}",
       size=28,
       color="white"
   )
   pregunta = ft.Text(
       "Choose a category and value!",
       size=24,
       color="white",
       text_align="center"
   )
   question_image = ft.Image(
       src="",
       width=300,
       height=200,
       fit="contain",
       visible=False
   )
   resultado = ft.Text(
       "",
       size=22,
       weight="bold"
   )
   correctas = 0
   incorrectas = 0
   right_text = ft.Text(
       f"Right Answers: {correctas}",
       size=22,
       color="lightgreen"
   )
   wrong_text = ft.Text(
       f"Wrong Answers: {incorrectas}",
       size=22,
       color="red"
   )
   entrada = ft.TextField(
       label="Type your answer",
       width=400,
       text_align="center"
   )
   actual = ""
   valor_actual = 0
   def empezar():
       resultado.value = "Game Started!"
       resultado.color = "green"
       page.update()
   def mostrar(cat, valor):
       nonlocal actual
       nonlocal valor_actual
       try:
           if (cat, valor) in usadas:
               resultado.value = "Question already used!"
               resultado.color = "orange"
               page.update()
               return
           texto, respuesta, imagen = questions[cat][valor]
           pregunta.value = f"{cat} - ${valor}\n\n{texto}"
           question_image.src = f"images/{imagen}"
           question_image.visible = True
           actual = respuesta.lower()
           valor_actual = valor
           usadas.append((cat, valor))
           page.update()
       except Exception as e:
           resultado.value = f"Error: {e}"
           resultado.color = "red"
           page.update()
   def sumar(num):
       global puntos
       puntos += num
       score.value = f"Score: {puntos}"
       page.update()
   def terminar():
       pregunta.value = "YOU WIN THE GAME!"
       resultado.value = "Congratulations!"
       resultado.color = "yellow"
       page.update()
   def revisar(e):
       global puntos
       nonlocal valor_actual
       nonlocal actual
       nonlocal correctas
       nonlocal incorrectas
       try:
           user = entrada.value.lower().strip()
           if actual == "":
               resultado.value = "Choose a question first!"
               resultado.color = "orange"
               page.update()
               return
           if user == "":
               resultado.value = "Please type an answer!"
               resultado.color = "orange"
               page.update()
               return
           if user == actual:
               right_sound.play()
               sumar(valor_actual)
               correctas += 1
               right_text.value = f"Right Answers: {correctas}"
               resultado.value = "Correct!"
               resultado.color = "green"
           else:
               wrong_sound.play()
               sumar(-valor_actual)
               incorrectas += 1
               wrong_text.value = f"Wrong Answers: {incorrectas}"
               resultado.value = f"Wrong! Correct answer: {actual}"
               resultado.color = "red"
           entrada.value = ""
           actual = ""
           valor_actual = 0
           question_image.visible = False
           page.update()
           if puntos >= 500:
               terminar()
       except Exception as e:
           resultado.value = f"Error: {e}"
           resultado.color = "red"
           page.update()
   def reset(e):
       global puntos
       nonlocal correctas
       nonlocal incorrectas
       nonlocal actual
       nonlocal valor_actual
       puntos = 0
       correctas = 0
       incorrectas = 0
       usadas.clear()
       score.value = f"Score: {puntos}"
       right_text.value = f"Right Answers: {correctas}"
       wrong_text.value = f"Wrong Answers: {incorrectas}"
       pregunta.value = "Choose a category and value!"
       resultado.value = ""
       entrada.value = ""
       question_image.visible = True
       actual = ""
       valor_actual = 0
       page.update()
   while juego:
       break
   tablero = []
   for cat in questions:
       cosas = [
           ft.Text(
               cat,
               size=24,
               weight="bold",
               color="yellow"
           )
       ]
       for valor in questions[cat]:
           boton = ft.ElevatedButton(
               f"${valor}",
               width=120,
               height=60,
               on_click=lambda e, c=cat, v=valor: mostrar(c, v)
           )
           cosas.append(boton)
       tablero.append(
           ft.Column(
               controls=cosas,
               horizontal_alignment=ft.CrossAxisAlignment.CENTER
           )
       )
   enviar = ft.ElevatedButton(
       "Submit Answer",
       width=250,
       height=50,
       on_click=revisar
   )
   reiniciar = ft.ElevatedButton(
       "Reset Game",
       width=250,
       height=50,
       on_click=reset
   )
   empezar()
   page.add(
       ft.Column(
           controls=[
               titulo,
               score,
               right_text,
               wrong_text,
               ft.Row(
                   controls=tablero,
                   alignment=ft.MainAxisAlignment.CENTER,
                   spacing=30
               ),
               ft.Divider(),
               pregunta,
               question_image,
               entrada,
               enviar,
               reiniciar,
               resultado
           ],
           horizontal_alignment=ft.CrossAxisAlignment.CENTER,
           spacing=20
       )
   )

ft.run(main=main, assets_dir="assets")